...
if(isset($_GET['delete']) && !empty($_GET['delete'])) {
    ...
    // Delete staff
    $delete_sql = "DELETE FROM chats WHERE id = ?";
    $stmt = $conn->prepare($delete_sql);
    $stmt->bind_param("i", $id);
    
    if($stmt->execute()) {
        $status_message = "Message has been deleted successfully!";
        $status_type = "success";
    } else {
        $status_message = "Error while deleting message : " . $conn->error;
        $status_type = "danger";
    }
...
if(!empty($search)) {
    ...
    $search_param = "%$search%";
    $search_condition = " WHERE (user LIKE ? OR user_email LIKE ? OR id LIKE ? OR created_at LIKE ?)";
    
    // Get total records
    $count_sql = "SELECT COUNT(*) as total FROM chats" . $search_condition;
    ...
    $sql = "SELECT * FROM chats" . $search_condition . " ORDER BY id DESC LIMIT ?, ?";
    ...
    // No search
    // Get total records
    $count_sql = "SELECT COUNT(*) as total FROM chats";
    $count_result = $conn->query($count_sql);
    $total_records = $count_result->fetch_assoc()['total'];
    $total_pages = ceil($total_records / $limit);
    
    // Get staff with pagination
    $sql = "SELECT * FROM chats ORDER BY id DESC LIMIT $start, $limit";
...
<div class="col-md-9 col-lg-10 main-content">
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Chat Management</h1>
    </div>
    ...
    <div class="row mb-3">
        <div class="col-md-6">
            <form action="" method="GET" class="d-flex">
                <input type="text" name="search" class="form-control me-2" placeholder="Search Chats..." value="<?php echo $search; ?>">
                    ...
                    <tbody>
                        <?php if($result && $result->num_rows > 0): ?>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="btn-group">
                                        <a href="edit-chat.php?id=<?php echo $row['id']; ?>" class="btn btn-sm btn-primary">
                                        ...
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    Are you sure you want to delete chat message from "<?php echo $row['user']; ?>"?
                                                </div>
